...
static const struct option OPTIONS[] = {
    ...
    { "update_rc", required_argument, NULL, 'r' + 'c'},   // support rc board to update 213
    { "update_rp", required_argument, NULL, 'r' + 'p'},   // support rp board to update 226
    { "update_rf", required_argument, NULL, 'r' + '-' + 'f'}, // support rf board to update 311
    { "update_rb", required_argument, NULL, 'r' + 'b'},   // support rb board to update 212 
    { "wipe_data", no_argument, NULL, 'w' },
    { "wipe_cache", no_argument, NULL, 'c' },
    { "show_text", no_argument, NULL, 't' },
    { "wipe_all", no_argument, NULL, 'w'+'a' },// 216
...
float PROGRESS_FRACTION_LEFT = 1.0;
...
main(int argc, char **argv) {
    ...
    const char *update_rc = NULL;
    const char *update_rp = NULL;
    const char *update_rf = NULL;
    const char *update_rb = NULL;
    ...
    while ((arg = getopt_long(argc, argv, "", OPTIONS, NULL)) != -1) {
        switch (arg) {
            ...
            case 'r'+'c': update_rc = optarg; break;
            case 'r'+'p': update_rp = optarg; break;
            case 'r'+'-'+'f': update_rf = optarg; break;
            case 'r'+'b': update_rb = optarg; break;
    ...
    //upgrade robot submodule
    int count = 0;
    UPGRADE_MODULE modules[6];
    if (update_rc != NULL)
    {
        modules[count].module = RC_UPGRADE_ID;
        modules[count].file   = update_rc;
        count++;
    }
    if (update_rp != NULL)
    {
        modules[count].module = RP_UPGRADE_ID;
        modules[count].file   = update_rp;
        count++;
    }
    if (update_rf != NULL)
    {
        modules[count].module = RF_UPGRADE_ID;
        modules[count].file   = update_rf;
        count++;
    }
    if (update_rb != NULL)
    {
        modules[count].module = RB_L_ARM_UPGRADE_ID;
        modules[count].file   = update_rb;
        count++;

        modules[count].module = RB_R_ARM_UPGRADE_ID;
        modules[count].file   = update_rb;
        count++;

        modules[count].module = RB_BODY_UPGRADE_ID;
        modules[count].file   = update_rb;
        count++;
    }
    ...
    if (count > 0) 
    {
        // set just_exit
        just_exit = true;
        PROGRESS_FRACTION_LEFT -= count * 0.1;

        printf("Upgrade robot subsystem...\n");
        robotSubSysUpdate(count, modules);
        printf("End upgrade robot subsystem.\n");
    }
    ...
    if (update_package != NULL) {
        ...
        //ui->SetBackground(RecoveryUI::INSTALLING_UPDATE);
        //ui->SetProgressType(RecoveryUI::DETERMINATE);
        ui->ShowProgress(PROGRESS_FRACTION_LEFT, 270);
